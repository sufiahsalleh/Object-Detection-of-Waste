...
public class MyProject {
    ...
    private static int nClasses = 3; //Class= Plastic, Metal, Glass
    private static List<String> labels;

    // parameters for the Yolo2OutputLayer
    private static int nBoxes = 5; //number of anchor boxes
    private static double lambdaNoObj = 0.5; //LFC for the no object confident component of loss function
    private static double lambdaCoord = 5.0; //LFC for position and size for component of the lost function
    private static double detectionThreshold = 0.2; //value used in obj detection to measure the overlap of predicted vs actual bb, so model will never return prediction lower than this value
    private static double[][] priorBoxes = {{1, 3}, {2.5, 6}, {3, 4}, {3.5, 8}, {4, 9}}; //predefined bounding box

    // hyperparameters for training
    private static int batchSize = 2; //Defines the number of samples to work through before updating the internal model parameters.
    private static int nEpochs = 150; //Defines the number times that the learning algorithm will work through the entire training dataset.
    private static double learningRate = 1e-4; //Define the number that control the speed of training.

    // model
    private static File modelFilename = new File(System.getProperty("user.dir"), "generated-models/ImageObjectDetection_tinyyolo.zip");
    private static ComputationGraph model;

    // bounding boxes
    private static Frame frame = null;
    private static final Scalar RED = RGB(255, 0, 0); //glass
    private static final Scalar GREEN = RGB(0, 255, 0); //metal
    private static final Scalar BLUE = RGB(0, 0, 255); //plastic
    ...
    public static void main(String[] args) throws Exception {
        // preparing dataset
        ImageIterator.setup();
        RecordReaderDataSetIterator trainIter = ImageIterator.trainIterator(batchSize);
        RecordReaderDataSetIterator testIter = ImageIterator.testIterator(1);
        ...
        if (modelFilename.exists()) {
            ...
            FineTuneConfiguration fineTuneConf = getFineTuneConfiguration(); //Utilize pre-trained networks to recognize classes they were not originally trained on.

            model = getComputationGraph(pretrained, priors, fineTuneConf);
            System.out.println(model.summary(
                    InputType.convolutional(
                            ImageIterator.yoloHeight,
                            ImageIterator.yoloWidth,
                            nClasses)
                    )
            );
    ...
    private static ComputationGraph getComputationGraph(ComputationGraph pretrained, INDArray priors, FineTuneConfiguration fineTuneConf) {
        return new TransferLearning.GraphBuilder(pretrained)
                ...
                .setFeatureExtractor("leaky_re_lu_7") //activation function
                ...
                thread = new Thread(() -> {
                    while (frame != null) {
                        try {
                            ...
                            resize(rawImage, resizeImage, new Size(ImageIterator.yoloWidth, ImageIterator.yoloHeight));
